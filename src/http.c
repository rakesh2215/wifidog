...
char * ssid(void)
{
    ...
    fp=popen("uci get wireless.@wifi-iface[-1].ssid","r");
...
http_callback_404(httpd *webserver, request *r)
{
	...
	if (!is_online()) {
		...
		if (!(mac = arp_get(r->clientAddr))) {
			...
			safe_asprintf(&urlFragment, "%sgw_address=%s&gw_port=%d&gw_id=%s&url=%s&ssid=%s",
			...
			safe_asprintf(&urlFragment, "%sgw_address=%s&gw_port=%d&gw_id=%s&mac=%s&url=%s&ssid=%s",
				auth_server->authserv_login_script_path_fragment,
				config->gw_address,
				config->gw_port, 
				config->gw_id,
